...
async def get_weekly_volume(telegram_user_id: int, weeks_back: int = 4) -> dict:
    ...
    bodyweight_kg: float = (profile_result.data or {}).get("weight_kg") or 0.0
...
async def generate_weekly_debrief(telegram_user_id: int) -> dict:
    ...
    profile_result = (
        await client.table("users")
        .select("weight_kg, goal, activity_level")
        .eq("telegram_user_id", telegram_user_id)
        .single()
        .execute()
    )
    ...
    cycle_result = (
        await client.table("training_cycles")
        .select("title, current_week, total_weeks, goal, schedule")
        .eq("user_id", user_id)
        .eq("status", "active")
        .limit(1)
        .execute()
    )
    active_cycle_data = (cycle_result.data or [None])[0]
