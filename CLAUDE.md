...
## База знаний
Раньше вёлась в Obsidian vault через MCP (сервер: obsidian). С 2026-08-14
obsidian-mcp-server в рабочем окружении недоступен (`fetch failed` — не поднят
Local REST API плагин, это не режим read-only, а полный обрыв связи) — по решению
пользователя документация ведётся в **`docs/`** этого репозитория плюс в графе
**`mcp__memory`** (knowledge graph, сущность `fitnessAI_v2` и связанные с ней).
Если Obsidian снова станет доступен — можно вернуться к нему, но по умолчанию
используй `docs/` + `mcp__memory`.

### Разделы
- **`docs/database.md`** — схема БД, `db/client.py` API, грабли asyncpg
- **`docs/deployment.md`** — Railway-инфраструктура (проект, сервисы, env vars)
- **`docs/daily-changes.md`** — лог изменений по дням
- **`docs/use_cases.md`**, **`docs/subagent-router-refactor.md`** — фичи
- **`mcp__memory`** — структурированные факты о проекте (`search_nodes`/`read_graph`
  с сущностью `fitnessAI_v2`), полезно перед началом задачи и для «протухших» фактов

## Алгоритм работы с задачей

1. `mcp__memory__search_nodes("fitnessAI")` — быстрый контекст по проекту
2. Прочитай релевантные файлы в `docs/`
3. Изучи существующий код в проекте если нужно
4. Реализуй задачу, следуя архитектуре из базы знаний
5. Обнови соответствующие файлы в `docs/` (database, deployment — если менялись)
   через `mcp__filesystem`
6. Запиши изменения в `docs/daily-changes.md` и добавь наблюдения в `mcp__memory`

**Всегда**, а не только в начале задачи:
- Если в процессе работы стало непонятно, как что-то устроено в проекте (архитектура,
  схема БД, деплой, статус фичи) — сначала проверь `mcp__memory__search_nodes` и
  `docs/`, прежде чем гадать или спрашивать пользователя.
- Любое значимое изменение (новая архитектура, схема БД, деплой, найденный и
  исправленный баг) — сразу фиксируй в `docs/daily-changes.md` + `mcp__memory`, не
  откладывай на «потом»; в конце сессии легко забыть.
- В начале **каждой новой сессии** SessionStart-хук (`.claude/settings.json`)
  автоматически подсовывает в контекст хвост `docs/daily-changes.md` — не нужно
  читать его вручную, но перепроверяй `docs/database.md`/`docs/deployment.md`,
  если задача их касается (хук их не включает, они не по дням, а по темам).
...
- LangGraph (агент)
- PostgreSQL (Railway addon, доступ через `asyncpg` напрямую — без Supabase/PostgREST, см. `docs/database.md`)
...
- `docs/` + `mcp__memory` — источник истины о проекте, держи их актуальными

## Obsidian MCP
Vault name: **fitnessai** (строчными буквами) — используй это имя, если/когда
obsidian-mcp-server снова станет доступен. На 2026-08-14 недоступен (`fetch failed`),
документация временно (или насовсем, по решению пользователя) ведётся через
`docs/` + `mcp__memory`, см. раздел «База знаний» выше.
